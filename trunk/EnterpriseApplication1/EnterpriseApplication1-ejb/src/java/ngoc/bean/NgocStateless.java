...
//    @PersistenceContext(unitName = "20130414eprojectSem4-ejbPU")
    ...
    public boolean updateHotelWithImage(int hotelID, String hotelName, String street, int star, String description, String image, int price) {
        try {
            Hotel hotel=searchReturn1Hotel(hotelID);//get Car that need to update
            hotel.setHotelName(hotelName);//set new value
            hotel.setHotelStreet(street);//set new value
            hotel.setHotelStar(star);//set new value
            hotel.setHotelDescription(description);//set new value
            hotel.setHotelImage(image);//set new value
            hotel.setHotelPrice(price);//set new value
            persist(hotel);//update to database
        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;//must have
    }
    public boolean updateHotelNoImage(int hotelID, String hotelName, String street, int star, String description, int price) {
        try {
            Hotel hotel=searchReturn1Hotel(hotelID);//get Car that need to update
            hotel.setHotelName(hotelName);//set new value
            hotel.setHotelStreet(street);//set new value
