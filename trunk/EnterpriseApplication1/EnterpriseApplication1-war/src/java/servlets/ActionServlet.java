...
import ngoc.bean.NgocStatefulRemote;
import ngoc.bean.NgocStatelessRemote;
...
public class ActionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            NgocStatefulRemote statefulRemote = (NgocStatefulRemote) ctx.lookup("NgocStatefulRemote");
...
//                request.setAttribute("$to", request.getParameter("$to"));
                ...
                } else if (statefulRemote.checkQuantity($pickup, $dropoff, $quantity, $carID)) {
                ...
                if (session.getAttribute("carCart") != null) {//if theres item in cart
                    statefulRemote = (NgocStatefulRemote) session.getAttribute("carCart");//get cart
                    for (int i = 0; i < statefulRemote.get$length(); i++) {//get all cart item
                        if ($carID == statefulRemote.$get$carID(i)//if car existed and not removed
                                && !statefulRemote.get$orderDTO()[i].is$deleted()) {
                ...
                statefulRemote.addOrderItem($carID, $model, $price, $quantity,
                        $inStock, $pickup, $dropoff, $driver);
                session.setAttribute("carCart", statefulRemote);//set session
                ...
                statefulRemote = (NgocStatefulRemote) session.getAttribute("carCart");//get session
                int $carID = statefulRemote.get$orderDTO()[i].get$carID();//get carID
                ...
                if (session.getAttribute("carCart") != null) {//if session not null
                    ...
                    statefulRemote.updateItem(i, $quantity, $pickup, $dropoff, $driver, false);
                }
                session.setAttribute("carCart", statefulRemote);//set session
                request.getRequestDispatcher("carShoppingCart.jsp").forward(request, response);//redirect
                ...
                if (session.getAttribute("carCart") != null) {//if cart not null
                    ...
                    statefulRemote.updateItem(i, 0, new Date(), new Date(), false, true);
                }
                session.setAttribute("carCart", statefulRemote);//set session
                request.getRequestDispatcher("carShoppingCart.jsp").forward(request, response);//redirect
                ...
                for (int i = 0; i < statefulRemote.get$length(); i++) {//get all cart item
                    if (!statefulRemote.get$orderDTO()[i].is$deleted()) {//if item is not removed from cart
                        //get orderCost by all car totalCost (price*quantity*(number of date book))
                        int $driverCost = 0;
                        if (statefulRemote.get$orderDTO()[i].is$driver()) {
                            $driverCost = 10;
                        }
                        $orderCost += (statefulRemote.$get$price(i)
                                * statefulRemote.get$orderDTO()[i].get$quantity() + $driverCost) * (int) ((statefulRemote.get$orderDTO()[i].get$dropoff().getTime()
                                - statefulRemote.get$orderDTO()[i].get$pickup().getTime())
                ...
                for (int i = 0; i < statefulRemote.get$length(); i++) {//get all cart item
                    if (!statefulRemote.get$orderDTO()[i].is$deleted()) {//if item is not removed from cart
                        $pickup = statefulRemote.get$orderDTO()[i].get$pickup();//get cart item value
                        $dropoff = statefulRemote.get$orderDTO()[i].get$dropoff();//get cart item value
                        $quantity = statefulRemote.get$orderDTO()[i].get$quantity();//get cart item value
                        $carID = statefulRemote.get$orderDTO()[i].get$carID();//get cart item value
                        //re-check quantity in database
                        if (statefulRemote.checkQuantity($pickup, $dropoff, $quantity, $carID)) {
                ...
                Date $orderDate = statefulRemote.insertOrder($userID, $orderCost);

                //get carOrderID by orderDate of the order just insert in database
                int $carOrderID = statefulRemote.searchReturn1($orderDate).getCarOrderID();
                //insert carOrderDetail
                for (int i = 0; i < statefulRemote.get$length(); i++) {//get all cart item
                    if (!statefulRemote.get$orderDTO()[i].is$deleted()) {//if item is not removed from cart
                        $carID = statefulRemote.get$orderDTO()[i].get$carID();//get cart item value
                        $quantity = statefulRemote.get$orderDTO()[i].get$quantity();//get cart item value
                        $driver = statefulRemote.get$orderDTO()[i].is$driver();//get cart item value
                        $pickup = statefulRemote.get$orderDTO()[i].get$pickup();//get cart item value
                        $dropoff = statefulRemote.get$orderDTO()[i].get$dropoff();//get cart item value
                        //get totalCost (price*quantity*(number of date book))
                        $totalCost = statefulRemote.$get$price(i)
                                * statefulRemote.get$orderDTO()[i].get$quantity() * (int) ((statefulRemote.get$orderDTO()[i].get$dropoff().getTime()
                                - statefulRemote.get$orderDTO()[i].get$pickup().getTime())
                                / (1000 * 60 * 60 * 24) + 1);//get time then divine by millisecond of 1 day

                        statefulRemote.insertOrderDetail($carOrderID, $carID, $quantity, $driver,
                ...
                CarOrder[] carOrder = (CarOrder[]) statefulRemote.searchCarOrder($userID, $from, $to);
                ...
                CarReportDTO[] top10 = statefulRemote.report($from, $to);
                ...
                statefulRemote.cancelOrder($carOrderID);
                ...
                if (session.getAttribute("admin") != null) {//if not login
                    statefulRemote = (NgocStatefulRemote) session.getAttribute("admin");//get session
                }
                //check login admin, if login successful redirect to admin page
                if (statefulRemote.adminLogin($username, $password)) {
                    session.setAttribute("admin", statefulRemote);//set session
...
//                int paging = Integer.parseInt(request.getParameter("paging"));
                ...
                request.getRequestDispatcher("adminHotelSearch.jsp").forward(request, response);//redirect
            } else if (action.equals("update hotel")) {//if update button
                ut.begin();//begin transaction
                int $hotelID = Integer.parseInt(request.getParameter("$hotelID"));//parse input value
                String $name = request.getParameter("$name");//get input value
                String $street = request.getParameter("$street");//get input value
                int $star = Integer.parseInt(request.getParameter("$star"));//parse input value
                ...
                int $price = Integer.parseInt(request.getParameter("$price"));//parse input value
                remote.updateHotelWithImage($hotelID, $name, $street, $star, $description, $image, $price);//update new detail to database
                request.getRequestDispatcher("adminCarSearch.jsp").forward(request, response);
                ut.commit();
            } else {
