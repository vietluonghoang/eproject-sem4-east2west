 ...
 * @author Tuan Ngoc
 */
public class NgocServlet extends HttpServlet {
...
//                request.setAttribute("$to", request.getParameter("$to"));
                ...
                } else if (ngocStatefulRemote.checkQuantity($pickup, $dropoff, $quantity, $carID)) {
                ...
                if (session.getAttribute("carCart") != null) {//if theres item in cart
                    ngocStatefulRemote = (NgocStatefulRemote) session.getAttribute("carCart");//get cart
                    for (int i = 0; i < ngocStatefulRemote.get$length(); i++) {//get all cart item
                        if ($carID == ngocStatefulRemote.$get$carID(i)//if car existed and not removed
                                && !ngocStatefulRemote.get$orderDTO()[i].is$deleted()) {
                ...
                ngocStatefulRemote.addOrderItem($carID, $model, $price, $quantity,
                        $inStock, $pickup, $dropoff, $driver);
                session.setAttribute("carCart", ngocStatefulRemote);//set session
                ...
                ngocStatefulRemote = (NgocStatefulRemote) session.getAttribute("carCart");//get session
                int $carID = ngocStatefulRemote.get$orderDTO()[i].get$carID();//get carID
                ...
                if (session.getAttribute("carCart") != null) {//if session not null
                    ...
                    ngocStatefulRemote.updateItem(i, $quantity, $pickup, $dropoff, $driver, false);
                }
                session.setAttribute("carCart", ngocStatefulRemote);//set session
                request.getRequestDispatcher("carShoppingCart.jsp").forward(request, response);//redirect
                ...
                if (session.getAttribute("carCart") != null) {//if cart not null
                    ...
                    ngocStatefulRemote.updateItem(i, 0, new Date(), new Date(), false, true);
                }
                session.setAttribute("carCart", ngocStatefulRemote);//set session
                request.getRequestDispatcher("carShoppingCart.jsp").forward(request, response);//redirect
                ...
                for (int i = 0; i < ngocStatefulRemote.get$length(); i++) {//get all cart item
                    if (!ngocStatefulRemote.get$orderDTO()[i].is$deleted()) {//if item is not removed from cart
                        //get orderCost by all car totalCost (price*quantity*(number of date book))
                        int $driverCost = 0;
                        if (ngocStatefulRemote.get$orderDTO()[i].is$driver()) {
                            $driverCost = 10;
                        }
                        $orderCost += (ngocStatefulRemote.$get$price(i)
                                * ngocStatefulRemote.get$orderDTO()[i].get$quantity() + $driverCost) * (int) ((ngocStatefulRemote.get$orderDTO()[i].get$dropoff().getTime()
                                - ngocStatefulRemote.get$orderDTO()[i].get$pickup().getTime())
                ...
                for (int i = 0; i < ngocStatefulRemote.get$length(); i++) {//get all cart item
                    if (!ngocStatefulRemote.get$orderDTO()[i].is$deleted()) {//if item is not removed from cart
                        $pickup = ngocStatefulRemote.get$orderDTO()[i].get$pickup();//get cart item value
                        $dropoff = ngocStatefulRemote.get$orderDTO()[i].get$dropoff();//get cart item value
                        $quantity = ngocStatefulRemote.get$orderDTO()[i].get$quantity();//get cart item value
                        $carID = ngocStatefulRemote.get$orderDTO()[i].get$carID();//get cart item value
                        //re-check quantity in database
                        if (ngocStatefulRemote.checkQuantity($pickup, $dropoff, $quantity, $carID)) {
                ...
                Date $orderDate = ngocStatefulRemote.insertOrder($userID, $orderCost);

                //get carOrderID by orderDate of the order just insert in database
                int $carOrderID = ngocStatefulRemote.searchReturn1($orderDate).getCarOrderID();
                //insert carOrderDetail
                for (int i = 0; i < ngocStatefulRemote.get$length(); i++) {//get all cart item
                    if (!ngocStatefulRemote.get$orderDTO()[i].is$deleted()) {//if item is not removed from cart
                        $carID = ngocStatefulRemote.get$orderDTO()[i].get$carID();//get cart item value
                        $quantity = ngocStatefulRemote.get$orderDTO()[i].get$quantity();//get cart item value
                        $driver = ngocStatefulRemote.get$orderDTO()[i].is$driver();//get cart item value
                        $pickup = ngocStatefulRemote.get$orderDTO()[i].get$pickup();//get cart item value
                        $dropoff = ngocStatefulRemote.get$orderDTO()[i].get$dropoff();//get cart item value
                        //get totalCost (price*quantity*(number of date book))
                        $totalCost = ngocStatefulRemote.$get$price(i)
                                * ngocStatefulRemote.get$orderDTO()[i].get$quantity() * (int) ((ngocStatefulRemote.get$orderDTO()[i].get$dropoff().getTime()
                                - ngocStatefulRemote.get$orderDTO()[i].get$pickup().getTime())
                                / (1000 * 60 * 60 * 24) + 1);//get time then divine by millisecond of 1 day

                        ngocStatefulRemote.insertOrderDetail($carOrderID, $carID, $quantity, $driver,
                ...
                CarOrder[] carOrder = (CarOrder[]) ngocStatefulRemote.searchCarOrder($userID, $from, $to);
                ...
                CarReportDTO[] top10 = ngocStatefulRemote.report($from, $to);
                ...
                ngocStatefulRemote.cancelOrder($carOrderID);
                ...
                if (session.getAttribute("admin") != null) {//if not login
                    ngocStatefulRemote = (NgocStatefulRemote) session.getAttribute("admin");//get session
                }
                //check login admin, if login successful redirect to admin page
                if (ngocStatefulRemote.adminLogin($username, $password)) {
                    session.setAttribute("admin", ngocStatefulRemote);//set session
