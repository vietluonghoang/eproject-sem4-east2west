...
<%
    ...
    try {
        ...
        if (!isMultiPart) {
            ...
            int $hotelID = 0;
            if ("update".equals($which)) {
                $hotelID = Integer.parseInt((String) params.get("$hotelID"));
            }
            if ($star < 0 || $star > 5) {
            ...
            if ("update".equals($which)) {
                if (image1.equals("")||image1==null) {
                    remote.updateHotelNoImage($hotelID, $name, $street, $star, $description, $price);
                    request.getRequestDispatcher("adminHotelSearch.jsp").forward(request, response);
                } else {
                    remote.updateHotelWithImage($hotelID, $name, $street, $star, $description, $image, $price);
                    request.getRequestDispatcher("adminHotelSearch.jsp").forward(request, response);
                }
            } else {
